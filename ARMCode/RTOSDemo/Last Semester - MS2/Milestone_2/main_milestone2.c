...
// Include file for milestone 2 tasks
...
int main( void )
{
	...
	vStartConductorTask(&conductorData,mainCONDUCTOR_TASK_PRIORITY,&vtI2C0,&i2cData);

	starti2cTask(&i2cData,mainI2C_TASK_PRIORITY,&vtI2C0,&vtLCDdata);
	// Here we set up a timer that will send messages to the i2c task.  The timer will determine how often to send an i2c request
